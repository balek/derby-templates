...
ParentWrapper.prototype.get = function(context, unescaped) {
  var value = (this.expression || this.template).get(context.forViewParent(), unescaped);
  if (value instanceof ParentWrapper) {
    value = value.get(context.forViewParent(), unescaped);
  }
  return value;
};
